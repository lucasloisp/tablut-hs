...
{-
    La flecha <- es una "asignación" de Maybe a en una "variable" a
        ...
        Si es Just la "variable" toma el valor a SIN EL JUST
-}
...
-- Funciones mencionadas en la cátedra
...
beginning = TablutGame ShieldPlayer t -- (f1, f2, f3, f4, f5, f6, f7, f8, f9)
    where
        t = ((Vacia, Vacia, Vacia, PeonEspada, PeonEspada, PeonEspada, Vacia, Vacia, Vacia)
            ...
            ,(Vacia, Rey, Vacia, Vacia, PeonEscudo, Vacia, Vacia, Vacia, Vacia)
            ,(PeonEspada, Vacia, Vacia, Vacia, PeonEscudo, Vacia, Vacia, Vacia, PeonEspada)
            ,(PeonEspada, PeonEspada, PeonEscudo, PeonEscudo, Vacia, PeonEscudo, PeonEscudo, PeonEspada, PeonEspada)
            ,(PeonEspada, Vacia, Vacia, Vacia, PeonEscudo, Vacia, Vacia, Vacia, PeonEspada)
...
-- Da todas las acciones posibles para cada jugador (vacía para el inactivo)
...
instance Show TablutAction where
    ...
    show (Mover Vertical (i, j) d) = algebraica i j ++ "/" ++ algebraica (i+d) j
    show (Mover Horizontal (i, j) d) = algebraica i j ++ "/" ++ algebraica i (j+d)


algebraica :: Int -> Int -> String
algebraica i j = letra j:show i
    where letra 0 = 'a'
          letra 1 = 'b'
          letra 2 = 'c'
          letra 3 = 'd'
...
readAlgebraic :: String -> (Int, Int)
readAlgebraic ('a':n) = (read n, 0)
readAlgebraic ('b':n) = (read n, 1)
readAlgebraic ('c':n) = (read n, 2)
readAlgebraic ('d':n) = (read n, 3)
readAlgebraic ('e':n) = (read n, 4)
readAlgebraic ('f':n) = (read n, 5)
readAlgebraic ('g':n) = (read n, 6)
readAlgebraic ('h':n) = (read n, 7)
readAlgebraic ('i':n) = (read n, 8)
readAlgebraic _ = error "No es una lectura válida"
...
readAction l = 
    Mover 
        ...
        ((j' - j) + (i' - i))
    where
        (i', j') = readAlgebraic to
        (i, j) = readAlgebraic fro
        (fro, _:to) = span (/= '/') l 
-- Errores de formateo, o movimientos imposibles (diagonal)
-- Ejemplo de uso : C3,D3 intenta mover una pieza de la posición C3 a la posición D3.
-- FALLA SI EL MOVIMIENTO ES ILEGAL.              
